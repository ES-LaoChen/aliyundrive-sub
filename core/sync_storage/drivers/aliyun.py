"""阿里云盘 Open API 驱动（移植自 TaoSync service/storage/drivers/aliyun.py）。

使用官方 OpenFile API（openapi.alipan.com），需要 client_id / client_secret /
refresh_token。包含 OAuth 刷新、分片上传、native copy、去重、限速、token 持久化回调。
逻辑与原作者保持一致；仅把 fileFingerprint 改为本地相对导入。
"""
import base64
import binascii
import hashlib
...
class AliyunDriveDriver(StorageDriver):
    ...
    def list(self, path, details=False):
        ...
        for item in self._list_id(parent["file_id"]):
            ...
            if details:
                entry["fingerprint"] = file_fingerprint(
                        "aliyun",
                        item.get("file_id"),
                        item.get("updated_at"),
